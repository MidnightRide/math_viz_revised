...
static inline void bezier_graph_update(struct BezierGraph *self,
                                       struct Bezier *bezier, u32 index) {
    ...
    if (glms_vec2_eqve(self->pos, state.window->mouse.position,
        ...
        if (state.window->mouse.buttons[GLFW_MOUSE_BUTTON_LEFT].pressed)
            bezier->selected_graph_index = index;
        bezier->allow_move_this_frame = false;
    }
}

static inline void selected_graph_update(struct BezierGraph *self,
                                         struct Bezier *bezier) {
    for (u32 i = 0; i < 4; ++i) {
        if (glms_vec2_eqve(self->p[i], state.window->mouse.position,
                           POINT_SIZE / 2) &&
    ...
    if (bezier->allow_move_this_frame) {
        if (state.window->mouse.buttons[GLFW_MOUSE_BUTTON_LEFT].down) {
            self->p[bezier->selected_point_index] =
                glms_vec2_lerp(self->p[bezier->selected_point_index],
                               state.window->mouse.position, 0.1f);
            bezier->graph->mesh_change_this_frame = true;
        }
    } else
        bezier->allow_move_this_frame = true;
}
...
static inline void bezier_graph_mesh(struct BezierGraph *self,
                                     struct Bezier *bezier) {
    struct Graph *graph = bezier->graph;
    for (u32 w = 0; w <= bezier->resolution; ++w) {
        f32 t = (f32)w / (f32)bezier->resolution;
        vec2s position =
            bezier4(self->p[0], self->p[1], self->p[2], self->p[3], t);
