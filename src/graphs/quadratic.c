...
static inline void quadratic_deserialize(struct Quadratic *self) {
    ...
    if (filesystem_file_exists(QUADRATIC_DATA_FILEPATH)) {
        ...
        fread(self->graphs, sizeof(struct QuadraticGraph), self->elements, file);
        fclose(file);
    }
...
void quadratic_selected_graph_update(struct QuadraticGraph *self, struct Quadratic *quadratic) {
    f32 speed = 1.0f;
    ...
    if (quadratic->allow_move_this_frame) {
        if (state.window->mouse.buttons[GLFW_MOUSE_BUTTON_LEFT].down) {
            self->a = lerpf(self->a, (mouse_pos.y - self->k)/squaref(mouse_pos.x-self->h), 0.1f);
            quadratic->graph->mesh_change_this_frame = true;
        }
    } else quadratic->allow_move_this_frame = true;
    ...
    self->a = clamp(self->a, -((f32)quadratic->resolution*state.window->size.y)/(state.window->size.x*2), ((f32)quadratic->resolution*state.window->size.y)/(state.window->size.x*2));
}

static inline void quadratic_graph_update(struct QuadraticGraph *self, struct Quadratic *quadratic, u32 index) {
    self->pos = (vec2s){{self->h, self->k}};
    
    if (glms_vec2_eqve(self->pos, state.window->mouse.position, POINT_SIZE/2)) {
        if (state.window->mouse.buttons[GLFW_MOUSE_BUTTON_LEFT].pressed) quadratic->selected_graph_index = index;
        quadratic->allow_move_this_frame = false;
    }
}

void quadratic_update(struct Quadratic *self) {
    quadratic_selected_graph_update(&self->graphs[self->selected_graph_index], self);
...
static inline void quadratic_graph_mesh(struct QuadraticGraph *self, struct Quadratic *quadratic) {
    struct Graph *graph = quadratic->graph;
    for (u32 i = 0; i <= quadratic->resolution; ++i)
    {
        f32 x = ((f32)i/(f32)quadratic->resolution)*state.window->size.x - state.window->size.x/2.f;
