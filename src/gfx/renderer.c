...
static inline void renderer_load_faces(struct Renderer *self, char *font_path) {
    ...
    if (FT_New_Face(self->ft, font_path, 0, &face)) {
        ...
        for (u8 c = 0; c < 128; c++) {
            ...
            struct Texture texture = texture_create_from_pixels(
...
static inline void renderer_init_text(struct Renderer *self, char *font_path) {
    ...
    vao_attr(self->text.vao, self->text.vbo, 0, 4, GL_FLOAT, 4 * sizeof(f32),
             0);
}
...
void renderer_init(struct Renderer *self) {
    *self = (struct Renderer){};
    ...
    self->shaders[ShaderType_basic_texture] = shader_create(
        ...
        (struct VertexAttr[]){{.index = 0, .name = "position"},
                              {.index = 1, .name = "uv"}});
    ...
    self->shaders[ShaderType_text] =
        shader_create("res/shaders/text.vs", "res/shaders/text.fs", 1,
                      (struct VertexAttr[]){
                          {.index = 0, .name = "vertex"},
                      });
    ...
    ortho_camera_init(
        &self->ortho_camera,
        (vec2s){{-state.window->size.x / 2.f, -state.window->size.y / 2.f}},
        (vec2s){{state.window->size.x / 2.f, state.window->size.y / 2.f}});
...
void renderer_quad_color(struct Renderer *self, vec2s size, vec4s color,
                         mat4s model) {
...
void renderer_quad_texture(struct Renderer *self, struct Texture texture,
                           vec2s size, vec4s color, vec2s uv_min, vec2s uv_max,
                           mat4s model) {
...
void renderer_text(struct Renderer *self, char *text, u32 text_length,
                   vec2s position, f32 scale, vec3s color) {
...
void renderer_aabb(struct Renderer *self, AABB aabb, vec4s color, mat4s model,
