...
static void _cursor_callback(GLFWwindow *handle, double xp, double yp) {
    vec2s p = {{xp - window.size.x / 2, window.size.y / 2 - yp}};
...
void window_create(FWindow init, FWindow destroy, FWindow tick, FWindow update,
                   FWindow render, FWindow resized) {
    window.init = init;
    ...
    window.handle =
        glfwCreateWindow(window.size.x, window.size.y, "Peanut", NULL, NULL);
...
static void button_array_tick(u64 n, struct Button *buttons) {
    for (u64 i = 0; i < n; ++i) {
...
static void button_array_update(u64 n, struct Button *buttons) {
    for (u64 i = 0; i < n; ++i) {
